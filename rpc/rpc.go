...
func DialMoneroServer(cert_path string, server string, port int, username string, password string, ignoreSslErrors bool) http.Client {
	...
	rpcClientOnce.Do(func() {
		...
		resp, err := MakeRequest(newRequest)
		if err != nil {
...
*
* @return *http.Response: The response from the Monero RPC server
 */
func MakeRequest(request MoneroRPCRequest) (*http.Response, error) {
	if rpcClient == nil {
...
/*
* ReadResponseBody reads the response body from the RPC response
* and decodes it into a MoneroRPCResponse object
*
* @param resp: The HTTP response from the Monero RPC server
*
* @return MoneroRPCResponse: The decoded response body
 */
func ReadResponseBody(resp *http.Response) (MoneroRPCResponse, error) {
	var result MoneroRPCResponse
	err := json.NewDecoder(resp.Body).Decode(&result)
	if err != nil {
		return result, err
